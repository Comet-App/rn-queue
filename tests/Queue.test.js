...
describe("Models/Queue", function () {
  ...
  it("#start(lifespan) ADVANCED TEST FULL (Multiple job names, job timeouts, concurrency, priority) - ONLY RUN IN NON-CI ENV: queue will process jobs with timeout set as expected until lifespan ends.", async () => {
    ...
    queue.createJob(
      ...
      {
        trackingName:
          "job10-concurrent-job-name-payloadTimeout(10000)-timeout(10100)-priority(0)", // THIS JOB WILL BE SKIPPED BY getConcurrentJobs() due to timeout too long.
        payloadTimeout: 10000,
        payloadOptionsTimeout: 10100, // Mirror the actual job options timeout in payload so we can use it for testing.
      },
      {
    ...
    queue.createJob(
      ...
      {
        trackingName:
          "job13-job-name-payloadTimeout(400)-timeout(500)-priority(0)", // THIS JOB WON'T BE RUN BECAUSE THE TIMEOUT IS 500 AND ONLY 950ms left by this pount. 950 - 500 = 450 and 500 remaining is min for job to be pulled.
        payloadTimeout: 400,
        payloadOptionsTimeout: 500, // Mirror the actual job options timeout in payload so we can use it for testing.
      },
      {
    ...
    queue.createJob(
      ...
      {
        trackingName:
          "job15-job-name-payloadTimeout(500)-timeout(600)-priority(0)", // THIS JOB WON'T BE RUN BECAUSE out of time!
        payloadTimeout: 500,
        payloadOptionsTimeout: 600, // Mirror the actual job options timeout in payload so we can use it for testing.
      },
      {
  ...
  it("#getConcurrentJobs() If worker concurrency is set to 3, getConcurrentJobs() should get up to 3 of same type of job as next job on top of queue.", async () => {
    ...
    JSON.parse(concurrentJobs[0].payload).should.deepEqual({
      random: "this is 1st random data",
    });
    JSON.parse(concurrentJobs[1].payload).should.deepEqual({
      random: "this is 2nd random data",
    });
    JSON.parse(concurrentJobs[2].payload).should.deepEqual({
      random: "this is 3rd random data",
    });
  ...
  it("#getConcurrentJobs() If worker concurrency is set to 10, but only 4 jobs of next job type exist, getConcurrentJobs() should only return 4 jobs.", async () => {
    ...
    JSON.parse(concurrentJobs[3].payload).should.deepEqual({
      random: "this is 4th random data",
    });
  ...
  it("#getConcurrentJobs() consecutive calls to getConcurrentJobs() gets new non-active jobs (and marks them active).", async () => {
    ...
    JSON.parse(thirdConcurrentJobs[0].payload).should.deepEqual({
      dummy: "1 data",
    });
    ...
    JSON.parse(fourthConcurrentJobs[0].payload).should.deepEqual({
      dummy: "3 data",
    });
  ...
  it("#processJob() increments failedAttempts counter until max attempts then fails on job failure.", async () => {
    ...
    // Process the jobs
    await Promise.all([
      queue.processJob(concurrentJobs[0]),
      queue.processJob(concurrentJobs[1]),
    ]);
    ...
    JSON.parse(secondConcurrentJobs[0].payload).should.deepEqual({
      dummy: "2 data",
    });
    JSON.parse(secondConcurrentJobs[1].payload).should.deepEqual({
      dummy: "3 data",
    });
      ...
      },
      {
        onSuccess: () => {
          ...
          if (!jobProcessed) {
            testFailed = true;
            throw new Error(
              "ERROR: onSuccess fired before job began processing."
            );
          }
        },
        onComplete: () => {
          ...
          if (!jobProcessed) {
            testFailed = true;
            throw new Error(
              "ERROR: onComplete fired before job began processing."
            );
          }
        },
      }
      ...
      },
      {
        onFailure: () => {
          ...
          if (!jobProcessStarted) {
            testFailed = true;
            throw new Error(
              "ERROR: onFailure fired before job began processing."
            );
          }
        },
        onFailed: () => {
          ...
          if (!jobProcessStarted) {
            testFailed = true;
            throw new Error(
              "ERROR: onFailed fired before job began processing."
            );
          }
        },
      }
   ...
   *
   * https://github.com/billmalarky/rn-queue/issues/15
   *
   */
